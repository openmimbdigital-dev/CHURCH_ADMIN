...
<body class="min-h-screen bg-slate-950 flex">
    ...
    <div class="hidden lg:flex lg:w-1/2 xl:w-3/5 relative overflow-hidden bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex-col items-center justify-center p-12">
        ...
        <div class="relative z-10 flex flex-col items-center text-center max-w-md">
            <div class="float">
                <img src="{{ asset('images/logo-initial.svg') }}"
                     alt="{{ config('app.name') }}"
                     class="h-28 w-28 drop-shadow-2xl mb-6 select-none">
            </div>
            <h1 class="text-4xl xl:text-5xl font-extrabold text-white tracking-tight leading-tight">
                Church <span class="text-indigo-400">CFE</span>
            </h1>
            <p class="mt-4 text-slate-300 text-lg font-light leading-relaxed">
                Gestión integral del proceso educativo y administrativo de la iglesia.
            </p>

            <div class="mt-10 grid grid-cols-3 gap-4 w-full">
                @foreach([
                    ['📚', 'Educación', 'Cursos, clases y seguimiento'],
                    ['👥', 'Miembros', 'Registro y pastoral'],
                    ['📊', 'Control', 'Reportes y administración'],
                ] as [$icon, $title, $desc])
        ...
        <p class="absolute bottom-6 text-xs text-slate-600 z-10">© {{ date('Y') }} {{ config('app.name') }} · Todos los derechos reservados</p>
    </div>
    ...
    <div class="w-full lg:w-1/2 xl:w-2/5 flex items-center justify-center p-6 sm:p-10 bg-white">
        <div class="w-full max-w-sm">
            ...
            <div class="flex flex-col items-center mb-8 lg:hidden">
                <img src="{{ asset('images/logo-initial.svg') }}" alt="{{ config('app.name') }}" class="h-16 w-16 mb-3">
                <h2 class="text-2xl font-bold text-slate-900">Church CFE</h2>
            </div>
            ...
            <p class="mt-8 text-center text-xs text-slate-400">
                © {{ date('Y') }} {{ config('app.name') }} · Sistema de gestión eclesiástica
            </p>
        </div>
    </div>
