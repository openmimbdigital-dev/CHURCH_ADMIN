<div class="relative mx-auto w-full min-w-0 max-w-[90rem] overflow-x-hidden">
    ...
    <nav class="mb-6 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-medium text-slate-500">
        ...
        <span class="font-semibold text-slate-900">Iglesias</span>
    </nav>
    ...
    <header class="mb-8 space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0 flex-1 border-l-4 border-primary-500 pl-4 sm:pl-5">
                ...
                <h1 class="mt-2 text-xl font-bold tracking-tight text-slate-900 md:text-2xl">Iglesias</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-600">
                    @if (auth()->user()->isSuperAdmin())
                        Gestión de iglesias de todos los negocios.
                    @else
                        Iglesias asociadas a tu organización.
                    @endif
                </p>
            </div>
            @can('churches.create')
                <x-ui.create-button :href="route('admin.churches.create')" class="w-full justify-center sm:w-auto">
                    Crear iglesia
                </x-ui.create-button>
            @endcan
        </div>
    ...
    <section class="panel-corporate">
        <div class="panel-corporate-header px-4 py-4 sm:px-5">
            <h2 class="font-semibold text-slate-800">Iglesias registradas</h2>
        </div>
        <x-ui.datatable-scroll class="p-0">
            <livewire:admin.churches.datatable-churches />
