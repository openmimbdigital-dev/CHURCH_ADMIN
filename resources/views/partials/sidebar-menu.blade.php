@foreach ($menuSections as $section)
    ...
    >
        <button
            ...
            class="flex w-full items-center justify-center rounded-lg px-2.5 py-2.5 text-sm font-medium text-slate-300 transition hover:bg-slate-800/80 hover:text-white"
            title="{{ $section->name }}"
        ...
        >
            @foreach ($section->activeItems as $item)
                <a
                    ...
                    class="flex items-center justify-center rounded-lg py-2.5 text-slate-300 transition hover:bg-slate-800 hover:text-white"
                    title="{{ $item->name }}"
                >
                    @if ($item->icon)
    ...
    {{-- Sidebar expandido (móvil drawer + escritorio) --}}
    <div x-show="showSidebarLabels()" x-cloak class="space-y-0.5">
        <button
            type="button"
            @click="toggleSectionNav('{{ $section->code }}')"
            class="flex w-full items-center gap-3 rounded-lg px-2.5 py-2.5 text-left text-sm font-medium text-slate-300 transition hover:bg-slate-800/80 hover:text-white"
        >
            <x-menu-icon :path="$section->icon" />
        ...
        <div x-show="isSectionOpen('{{ $section->code }}')" class="ml-4 mt-0.5 space-y-0.5 border-l border-slate-700/80 pl-3">
            @foreach ($section->activeItems as $item)
                <a
                    href="{{ $item->url ?? '#' }}"
                    @if(($item->url ?? '#') !== '#') wire:navigate @endif
                    class="flex items-center gap-2 rounded-lg px-2.5 py-2 text-sm transition
                        {{ request()->fullUrlIs(url($item->url ?? '#')) || request()->is(ltrim($item->url ?? '#', '/')) ? 'bg-slate-800/90 font-medium text-white' : 'text-slate-400 hover:bg-slate-800/60 hover:text-white' }}"
                    title="{{ $item->name }}"
                >
                    @if ($item->icon)
